...
def total_loss(fmap_real, logits_fake, fmap_fake, input_wav, output_wav, sample_rate=16000):
    ...
    loss = torch.tensor([0.0], requires_grad=True).to(logits_fake[0].device)
    l_t = torch.tensor([0.0], requires_grad=True).to(logits_fake[0].device)
    l_f = torch.tensor([0.0], requires_grad=True).to(logits_fake[0].device)
    l_g = torch.tensor([0.0], requires_grad=True).to(logits_fake[0].device)
    l_feat = torch.tensor([0.0], requires_grad=True).to(logits_fake[0].device)
    ...
    for i in range(5, 11):
        fft = Audio2Mel(win_length=2 ** i, hop_length=2 ** i // 4, n_mel_channels=64, sampling_rate=sample_rate, device=logits_fake[0].device)
...
def disc_loss(logits_real, logits_fake):
    ...
    """
    relu = torch.nn.ReLU()
    lossd = torch.tensor([0.0], requires_grad=True).to(logits_real[0].device)
